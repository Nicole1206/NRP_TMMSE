...
%         delta = 0;
%         adS = exp(j * kd * sin(DOAS + delta) * (1 : M)');
...
%         [rhoHKB_SVE, Rhkb_SVE] = HKBfunction(ad_est, R, M);
        ...
        %% method of DL-CGLS or the method of SMF       
        ad_norm = ad/sqrt(M);
        ...
        %% method of SVE
%         Ang_left = DOAS + delta - 5*pi/180;
%         Ang_right = DOAS + delta + 5*pi/180;
%         Ang_interval = 0.1 * pi / 180;
%         
%         % the estimate steering vector of SVE method
%         ad_est = SVEfunction(Ang_left,Ang_right,Ang_interval,kd, R);
%         Wsve = inv(R) * ad_est / (ad_est'* inv(R) * ad_est);
%         
%         SINR_sve(m) = 10 * log10(10^(EnerS/10) * abs(Wsve' * adS)^2 / real(Wsve' * Q * Wsve));
%         SOI_sve(m) = 10 * log10(abs(Wsve' * R0 * Wsve));
...
figure(1),
plot(SNR_list, abs(DL_glc_list),'r','linewidth',2)
...
figure(2),
plot(SNR_list, abs(DL_hkb_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{hkb}'},'FontName','Times New Roman','FontSize',20);

figure(3),
plot(SNR_list, abs(DL_cg_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{cg}'},'FontName','Times New Roman','FontSize',20);

figure(4)
plot(SNR_list, abs(DL_qc_list), 'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{qc}'},'FontName','Times New Roman','FontSize',20);

figure(5),
plot(SNR_list,abs(DL_glc_ls_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{ls}'},'FontName','Times New Roman','FontSize',20);

figure(6),
plot(SNR_list,abs(DL_glc_mwf_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
...
figure(70),
h = plot(SNR_list,SINR_opt_list,'k--',...
    SNR_list, SINR_GLC_list,'->',...
    SNR_list, SINR_GLC_SVE_list,'-.>',...
    SNR_list,SINR_HKB_list,'-o',...
    SNR_list,SINR_HKB_SVE_list,'-.o',...
    SNR_list, SINR_CG_list, '-v',...
    SNR_list, SINR_CG_SVE_list, '-.v',...
    SNR_list, SINR_QC_list,'-p',...
    SNR_list,SINR_GLC_LS_list,'-s',...
    SNR_list,SINR_GLC_LS_SVE_list,'-.s',...
    SNR_list,SINR_GLC_MWF_list,'-<',...
    SNR_list,SINR_GLC_MWF_SVE_list,'-.<',...
    SNR_list, SINR_SCB_list,'-d');
set(h, 'linewidth',2.5,'MarkerSize',6)
for idx = 1 : length(h)
    set(h(idx),'color',cell2mat(RGB_table(idx))/255);
end
xlabel('{SNRin}/dB','FontName','Times New Roman','FontSize',20);
ylabel('{SINR\out}/dB','FontName','Times New Roman','FontSize',20);
set(gca,'FontName','Times New Roman','FontSize',20); 
hleg = legend('SINR_{opt}','GLC','GLC-SVE','HKB','HKB-SVE','SMF','SMF-SVE','QC','LS-TMMSE','LS-TMMSE-SVE','MWF-TMMSE','MWF-TMMSE-SVE','SCB','Location','NorthWest');
set(hleg,'Fontsize',10);
% axis([N_list(1),N_list(15),min(SINR_SCB_list)-5,max(SINR_opt_list)+2])
...
figure(80),
h_elimsve = plot(SNR_list,SINR_opt_list,'k--',...
    SNR_list, SINR_GLC_list,'->',...
    SNR_list,SINR_HKB_list,'-o',...
    SNR_list, SINR_CG_list, '-v',...
    SNR_list, SINR_QC_list,'-p',...
...
ylabel('{SINR\out}/dB','FontName','Times New Roman','FontSize',20);
set(gca,'FontName','Times New Roman','FontSize',20); 
hleg = legend('SINR_{opt}','GLC','HKB','SMF','QC','LS-TMMSE','MWF-TMMSE','SCB','Location','NorthWest');
set(hleg,'Fontsize',10);
% axis([N_list(1),N_list(15),min(SINR_SCB_list)-5,max(SINR_opt_list)+2])
set (gcf,'Position',[10,10,1200,900])
nummarkers(h_elimsve, 10);
%% figures of the beampattern
% Wopt = (inv(R) * ad) / (ad' * inv(R) * ad);
% th = -pi/2 : pi/10000 : pi/2;
% for AngIdx = 1 : 1 : length(th),
%     adplot = exp(j * kd * sin(th(AngIdx)) * (1 : M)');
%     Pth(AngIdx) = 10 * log10(abs(adplot' * Wopt));
% end;
% figure;plot(th*180/pi,Pth, 'b');
% 
% Wopt = (inv(Rglc_LS) * ad) / (ad' * inv(Rglc_LS) * ad);
% th = -pi/2 : pi/10000 : pi/2;
% for AngIdx = 1 : 1 : length(th),
%     adplot = exp(j * kd * sin(th(AngIdx)) * (1 : M)');
%     Pth(AngIdx) = 10 * log10(abs(adplot' * Wopt));
% end;
% hold on;plot(th*180/pi,Pth,'r');
% 
% % Wopt = (inv(Rcg) * ad) / (ad' * inv(Rcg) * ad);
% th = -pi/2 : pi/10000 : pi/2;
% for AngIdx = 1 : 1 : length(th),
%     adplot = exp(j * kd * sin(th(AngIdx)) * (1 : M)');
%     Pth(AngIdx) = 10 * log10(abs(adplot' * Wcg));
% end;
% hold on;plot(th*180/pi,Pth,'g');
% 
% % Wopt = (inv(R) * ad) / (ad' * inv(R) * ad);
% th = -pi/2 : pi/10000 : pi/2;
% for AngIdx = 1 : 1 : length(th),
%     adplot = exp(j * kd * sin(th(AngIdx)) * (1 : M)');
%     Pth(AngIdx) = 10 * log10(abs(adplot' * Wqc));
% end;
% hold on;plot(th*180/pi,Pth, 'm');