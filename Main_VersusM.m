...
M_list = 10 : 10 : 100;          %Only change the value of M to obtain the results of figure 2.
N = 120;
...
%         [rhoHKB_SVE, Rhkb_SVE] = HKBfunction(ad_est, R, M);
        ...
        %% method of QC
        Ang_delta = asin(1/M);
        Ang_left_qc = DOAS  - Ang_delta;
        Ang_right_qc = DOAS  + Ang_delta;
        TestPoint = 4;     
        DLqc = 10*M; % initiate the DLL
    ...
    % DL
    DL_glc_ls_list(MIdx) = mean(DL_glc_ls);
    DL_glc_mwf_list(MIdx) = mean(DL_glc_mwf);
    DL_glc_list(MIdx) = mean(DL_glc);
    DL_hkb_list(MIdx) = mean(DL_hkb);
    DL_cg_list(MIdx) = mean(DL_cg);
    DL_qc_list(MIdx) = mean(DL_qc);
    
    % SINR
    SINR_opt_list(MIdx) = mean(SINR_opt);
    SINR_GLC_LS_list(MIdx) = mean(SINR_GLC_LS);
    SINR_GLC_LS_SVE_list(MIdx) = mean(SINR_GLC_LS_SVE);    
    SINR_GLC_MWF_list(MIdx) = mean(SINR_GLC_MWF);
    SINR_GLC_MWF_SVE_list(MIdx) = mean(SINR_GLC_MWF_SVE);  
    SINR_GLC_list(MIdx) = mean(SINR_GLC);
    SINR_GLC_SVE_list(MIdx) = mean(SINR_GLC_SVE);    
    SINR_HKB_list(MIdx) = mean(SINR_HKB);
    SINR_HKB_SVE_list(MIdx) = mean(SINR_HKB_SVE);    
    SINR_CG_list(MIdx) = mean(SINR_cg);
    SINR_CG_SVE_list(MIdx) = mean(SINR_cg_sve);   
    SINR_QC_list(MIdx) = mean(SINR_qc);
    SINR_SCB_list(MIdx) = mean(SINR_SCB);
    
    % SOI
    SOI_opt_list(MIdx) = mean(SOI_opt);    
    SOI_GLC_LS_list(MIdx) = mean(SOI_GLC_LS);
    SOI_GLC_LS_SVE_list(MIdx) = mean(SOI_GLC_LS_SVE);    
    SOI_GLC_MWF_list(MIdx) = mean(SOI_GLC_MWF);
    SOI_GLC_MWF_SVE_list(MIdx) = mean(SOI_GLC_MWF_SVE);   
    SOI_GLC_list(MIdx) = mean(SOI_GLC);
    SOI_GLC_SVE_list(MIdx) = mean(SOI_GLC_SVE);    
    SOI_HKB_list(MIdx) = mean(SOI_HKB);
    SOI_HKB_SVE_list(MIdx) = mean(SOI_HKB_SVE);    
    SOI_CG_list(MIdx) = mean(SOI_cg);
    SOI_CG_SVE_list(MIdx) = mean(SOI_cg_sve);    
    SOI_QC_list(MIdx) = mean(SOI_qc);
    SOI_SCB_list(MIdx) = mean(SOI_SCB);
end

%% figures of the DL value
figure(1),
plot(M_list, abs(DL_glc_list),'r','linewidth',2)
...
figure(2),
plot(M_list, abs(DL_hkb_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{hkb}'},'FontName','Times New Roman','FontSize',20);

figure(3),
plot(M_list, abs(DL_cg_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{cg}'},'FontName','Times New Roman','FontSize',20);

figure(4)
plot(M_list, abs(DL_qc_list), 'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{qc}'},'FontName','Times New Roman','FontSize',20);

figure(5),
plot(M_list,abs(DL_glc_ls_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
ylabel({'{\rho}_{ls}'},'FontName','Times New Roman','FontSize',20);

figure(6),
plot(M_list,abs(DL_glc_mwf_list),'r','linewidth',2)
set(gca,'FontName','Times New Roman','FontSize',20);
xlabel('Number of Snapshots','FontName','Times New Roman','FontSize',20);
...
figure(70),
h = plot(M_list,SINR_opt_list,'k--',...
    M_list, SINR_GLC_list,'->',...
    M_list, SINR_GLC_SVE_list,'-.>',...
    M_list,SINR_HKB_list,'-o',...
    M_list,SINR_HKB_SVE_list,'-.o',...
    M_list, SINR_CG_list, '-v',...
    M_list, SINR_CG_SVE_list, '-.v',...
    M_list, SINR_QC_list,'-p',...
    M_list,SINR_GLC_LS_list,'-s',...
    M_list,SINR_GLC_LS_SVE_list,'-.s',...
    M_list,SINR_GLC_MWF_list,'-<',...
    M_list,SINR_GLC_MWF_SVE_list,'-.<',...
    M_list, SINR_SCB_list,'-d');
set(h, 'linewidth',2.5,'MarkerSize',6)
for idx = 1 : length(h)
    set(h(idx),'color',cell2mat(RGB_table(idx))/255);
end
xlabel('Number of Sensors','FontName','Times New Roman','FontSize',20);
...
axis([M_list(1),M_list(end),min(SINR_SCB_list)-5,max(SINR_opt_list)+2])
...
figure(80),
h_elimsve = plot(M_list,SINR_opt_list,'k--',...
    M_list, SINR_GLC_list,'->',...
    M_list,SINR_HKB_list,'-o',...
    M_list, SINR_CG_list, '-v',...
    M_list, SINR_QC_list,'-p',...
